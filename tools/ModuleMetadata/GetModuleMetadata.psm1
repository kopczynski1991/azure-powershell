...
function Get-OutputTypeMetadata
{
    ...
    foreach ($Output in $Cmdlet.OutputType)
    {
        ...
        # Write-Host $Output.Type -ForegroundColor Green
...
function Get-ParameterMetadata
{
    ...
    foreach ($Parameter in $Cmdlet.Parameters.Values)
    {
        ...
        $ParameterMetadata = [Tools.Common.Models.ParameterMetadata]::New()
        $ParameterMetadata.Name = $Parameter.Name
        # Write-Host $Parameter.ParameterType -ForegroundColor Green
...
function Get-ModuleMetadata
{
    ...
    $ToolCommonDllPath = [System.IO.Path]::Combine($PSScriptRoot, "..", "..", "artifacts", "StaticAnalysis", "Tools.Common.dll")
    [Reflection.Assembly]::LoadFile($ToolCommonDllPath)
    if (!(Test-Path -Path $Path))
    {
        Write-Error "Fail to find path: $Path"
    }

    if (Test-Path -Path ([System.IO.Path]::Combine($Path, "generated")))
    {
        $IsGeneratedModule = $True
    }
    else
    {
        $IsGeneratedModule = $False
    }
    $Psd1Path = (Get-ChildItem -Path $Path -Filter *.psd1)[0].FullName
    $ModuleName = $Psd1Path.replace('\', '/').Split('/')[-1].replace('.psd1', '')

    if ($IsGeneratedModule)
    {
        ...
        $ModuleMetadata | ConvertTo-Json -depth 100 | Out-File $DumpJsonPath
        return $ModuleMetadata
    }
    else
    {
        $ParentDirectory = [System.IO.Directory]::GetParent($Path).FullName
        Write-Host $ParentDirectory -ForegroundColor Green
        [Tools.Common.Loaders.SharedAssemblyLoader]::Load($ParentDirectory)
        Import-LocalizedData -BaseDirectory $Path -FileName "$ModuleName.psd1" -BindingVariable ModuleMetadata
        $NestedModules = $ModuleMetadata.NestedModules
        if ($ModuleMetadata.RequiredModules.Count -ne 0)
        {
            $RequiredModules = $ModuleMetadata.RequiredModules | Foreach-Object { [System.IO.Path]::Combine($ParentDirectory, $_['ModuleName']) }
        }
        else
        {
            $RequiredModules = @()
        ...
        {
            $DllPath = [System.IO.Path]::Combine($Path, $NestedModule)
            $proxy = [Tools.Common.Loaders.CmdletLoader]::new()
            Write-Host $DllPath -ForegroundColor Green
            $Metadata = $proxy.GetModuleMetadata($DllPath, $RequiredModules)
            $Metadata.AssemblyFileName = $NestedModule
            $DumpJsonPath = [System.IO.Path]::Combine($Path, "$NestedModule.json")
            $Metadata | ConvertTo-Json -depth 100 | Out-File $DumpJsonPath
            $Result += $Metadata
        }
        return $Result
    }
